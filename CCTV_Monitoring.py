...
def save_snapshot(cam_id, cam_name, frame):
    ...
    SNAPSHOT_DIR = "C:\\Users\\USER\\Desktop\\cctv_blur_detection\\Blurr Signals"
...
def generate_no_signal_image(cam_id, cam_name):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"{cam_id}_{cam_name}_{timestamp}.jpg"
    snap_dir = "C:\\Users\\USER\\Desktop\\cctv_blur_detection\\No Singals"
    ...
    while True:
        ...
        for batch_cam_list in batches:
            ...
            if TRAINING_MODE or len(db_data) < num:
                ...
                # v_bh = [v for l in blur_hist for v in l]
                # global_blur = float(np.median(v_bh)) if v_bh else 150.0

                # v_brh = [v for l in bright_hist for v in l]
                # global_bright = float(np.median(v_brh)) if v_brh else 100.0

                # v_nh = [v for l in noise_hist for v in l]
                # global_noise = float(np.median(v_nh)) if v_nh else 20.0

                # v_ch = [v for l in contrast_hist for v in l]
                # global_contrast = float(np.median(v_ch)) if v_ch else 40.0
            ...
            while time.time() - batch_start < BATCH_DURATION:
                ...
                # v_blur = [b for b in blur_base if b > 0]
                # global_blur = float(np.median(v_blur)) if v_blur else 150.0

                # v_bright = [b for b in bright_base if b > 0]
                # global_bright = float(np.median(v_bright)) if v_bright else 100.0

                # v_noise = [b for b in noise_base if b > 0]
                # global_noise = float(np.median(v_noise)) if v_noise else 20.0

                # v_contrast = [b for b in contrast_base if b > 0]
                # global_contrast = float(np.median(v_contrast)) if v_contrast else 40.0
