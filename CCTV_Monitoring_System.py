...
def is_camera_online(url, timeout=1.0):
    try:
        ...
        # Strip credentials if present (split from right in case username/password contains '@')
        if "@" in temp:
            temp = temp.rsplit("@", 1)[1]
            
        # Strip path/query if present to isolate host and port
        host_port = temp.split("/", 1)[0]
...
def get_last_traverse_id():
    ...
    last_id = None
    try:
        cur.execute("SELECT last_traverse FROM category_wise_status WHERE category = 'blur detection'")
        row = cur.fetchone()
        if row:
            last_id = row[0]
    except Exception as e:
...
def update_last_traverse_id(cam_id):
    ...
    cur = conn.cursor()
    try:
        cur.execute("UPDATE category_wise_status SET last_traverse = %s WHERE category = 'blur detection'", (cam_id,))
        if cur.rowcount == 0:
            cur.execute("INSERT INTO category_wise_status (id, category, last_traverse) VALUES ((SELECT COALESCE(MAX(id), 0) + 1 FROM category_wise_status), 'blur detection', %s)", (cam_id,))
        conn.commit()
    except Exception as e:
...
def fetch_camera_master(start_cam_id=None, end_cam_id=None):
    conn = get_camera_conn()
    ...
    if end_cam_id is not None:
        camera_list = [c for c in camera_list if c['id'] <= end_cam_id]
...
def monitor_camera(start_cam_id=None, end_cam_id=None):
    ...
    if start_cam_id is None:
        ...
        if start_cam_id:
            print(f"[INFO] Resuming traversal from camera ID: {start_cam_id} (from DB history)")

    camera_master = fetch_camera_master(start_cam_id, end_cam_id)
    ...
    while True:
        ...
        for batch_cam_list in batches:
            ...
            for idx, cam in enumerate(batch_cam_list):
                streams.append(CameraStream(cam["rtsp"]))
                ...
                while time.time() - start_w < 5.0:
                    ...
                    cv2.putText(loading_frame, text1, ((grid_w - t1_size[0]) // 2, grid_h // 2 - 30), font, 1, (255, 255, 255), 2)
                    cv2.putText(loading_frame, text2, ((grid_w - t2_size[0]) // 2, grid_h // 2 + 20), font, 0.8, (0, 255, 255), 2)
            ...
            while True:
                ...
                for i in range(num):
                    ...
                    cv2.rectangle(f, (0, 0), (t_size[0] + 10, t_size[1] + 10), (0, 0, 0), -1)
                ...
                while len(frames) < rows * cols:
                    ...
                    if is_first_empty:
                        ...
                        cv2.rectangle(empty_f, (1, 1), (CELL_W-2, CELL_H-2), (100, 100, 100), 2)
                        
                        font = cv2.FONT_HERSHEY_SIMPLEX
                        cv2.putText(empty_f, "CCTV Health", (20, 50), font, 0.9, (255, 255, 255), 2)
                        cv2.putText(empty_f, "Monitoring System", (20, 80), font, 0.7, (200, 200, 200), 2)
                        ...
                        cv2.putText(empty_f, time_text, (20, 140), font, 0.8, t_color, 2)
                        cv2.putText(empty_f, f"Active Cameras: {num}", (20, 180), font, 0.6, (255, 255, 255), 1)
                        cv2.putText(empty_f, "[Space] Pause | [N] Skip", (10, 220), font, 0.45, (150, 150, 150), 1)
    ...
    print("Traversal completely finished. Resetting history to start from beginning next time.")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        try:
            ...
            print(f"Ending traversal at custom camera ID: {end_id}")
        except ValueError:
            print("Invalid end ID provided. Traversal will continue to the last camera.")
    monitor_camera(start_id, end_id)
